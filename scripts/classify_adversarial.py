...
def compare_results(df, original_predictions, adversarial_predictions, valid_samples):
    ...
    flipped_samples_df.to_csv('./data/Llama-3.2-3B-Instruct_flipped_samples.csv', index=False)
    print(
        f"Successfully exported {len(flipped_samples_df)} flipped samples to './data/Llama-3.2-3B-Instruct_flipped_samples.csv'")

    # 导出保留原义且成功翻转的样本到CSV文件
    preserved_flipped_df = df[(df['agreed_labels'] == 0) & (df['prediction_flipped'] == True)]
    preserved_flipped_df.to_csv('./data/Llama-3.2-3B-Instruct_preserved_flipped_samples.csv', index=False)
    print(
        f"Successfully exported {len(preserved_flipped_df)} preserved meaning flipped samples to './data/Llama-3.2-3B-Instruct_preserved_flipped_samples.csv'")
