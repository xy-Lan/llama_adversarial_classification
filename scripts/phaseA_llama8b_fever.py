#!/usr/bin/env python3
# phaseA_llama8b_fever.py  —  LoRA-8 微调 Llama-3-8B-Instruct on FEVER
...
class WikiCache:
    def __init__(self, cache_dir=None):
        ...
        self._idx   = {row["id"]: i for i, row in enumerate(wiki)}  # str key
        self._data  = wiki
...
def build_trainer(cfg) -> SFTTrainer:
    ...
    targs = TrainingArguments(
        ...
        logging_steps=100,
        save_strategy="epoch",
        gradient_checkpointing=True if cfg.grad_ckpt else False,
    )
...
def cli():
    ...
    ap.add_argument("--model_id", default="meta-llama/Llama-3.2-8B-Instruct")
    ap.add_argument("--token", default=None)
    ap.add_argument("--output_dir", default="./phaseA_llama8B")
    ap.add_argument("--batch", type=int, default=8)      # 单卡 H100 建议
    ap.add_argument("--grad_acc", type=int, default=4)
    ap.add_argument("--epochs", type=int, default=1)
    ap.add_argument("--lr", type=float, default=2e-4)
    ap.add_argument("--grad_ckpt", action="store_true", default=True,
                    help="默认开启以节省显存，可用 --no-grad_ckpt 关闭")
