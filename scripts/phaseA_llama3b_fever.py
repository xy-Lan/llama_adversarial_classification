...
def build_trainer(cfg) -> SFTTrainer:
    ...
    tok = AutoTokenizer.from_pretrained(
        ...
        add_eos_token=False,
        use_fast=False,
        use_auth_token=cfg.token or os.getenv("HF_TOKEN"),
    )
    ...
    lora_config = LoraConfig(
        ...
        target_modules=[
            "q_proj",
            "v_proj",
        ],  # Consider adding "k_proj", "o_proj" for Llama 3 if issues persist
        bias="none",
        lora_dropout=0.05,
    )
...
def cli():
    ap = argparse.ArgumentParser(
        description="Fine-tune Llama-3.2-3B for FEVER fact-checking (Llama 3.2 Prompt Format)"
    )
    ap.add_argument(
        "--model_id", default="meta-llama/Llama-3.2-3B-Instruct", help="Base model ID."
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--output_dir",
        default="./phaseA_llama3B_L3prompt",  # Suggest new dir for new prompt format
    ...
    ap.add_argument(
        "--cache_dir",
        default="/mnt/parscratch/users/acc22xl/huggingface",
        help="Directory for Hugging Face datasets cache.",
    )
