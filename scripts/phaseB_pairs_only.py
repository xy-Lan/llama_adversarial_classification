...
def load_adv_pairs(csv_path: str) -> Dataset:
    ...
    if missing_cols:
        raise ValueError(
            f"CSV file '{csv_path}' is missing required columns: {missing_cols}"
        )

    # --- 添加 pair_id (如果不存在) ---
    if "pair_id" not in df.columns:
        print(
            "Warning: 'pair_id' column not found in CSV. Generating pair IDs from index."
        )
        df["pair_id"] = df.index
    elif df["pair_id"].isnull().any():
        print(
            "Warning: 'pair_id' column contains null values. Filling with index where null."
        )
    ...
    try:
        ...
        print(
            "Warning: Could not convert 'pair_id' column to integer. Sorting might be affected."
        )
    ...
    def is_agreed_label_zero(label_value):
        if label_value is None:
            return False
        try:
            return float(label_value) == 0.0
        except (ValueError, TypeError):
            return False

    df_filtered = df[
        df["agreed_labels"].apply(is_agreed_label_zero)
    ].copy()  # Create a copy to avoid SettingWithCopyWarning
    # 创建副本以避免 SettingWithCopyWarning

    filtered_count = len(df_filtered)
    print(
        f"Filtered CSV based on 'agreed_labels == 0.0'. Original rows: {original_count}, Filtered pairs: {filtered_count}"
    )
    if filtered_count == 0:
        print(
            "Warning: No pairs remained after filtering for agreed_labels == 0.0. Check the input CSV."
        )
        # Return empty dataset if no pairs are left
        # 如果没有留下任何对，则返回空数据集
        return Dataset.from_dict(
            {"text": [], "is_adv": [], "pair_id": [], "labels": []}
        )
    ...
    print(
        f"Restructured data into {len(df_restructured)} individual samples (original and adversarial)."
    )
    ...
    missing_final_cols = [
        col for col in final_expected_cols if col not in df_restructured.columns
    ]
    if missing_final_cols:
        raise ValueError(
            f"Restructured DataFrame is missing expected columns: {missing_final_cols}"
        )
...
def adv_collator(features):
    ...
    def stack(name):
        ...
        if name not in features[0]:
            ...
            if not key_exists:
                raise KeyError(
                    f"Field '{name}' not found in any sample of the batch. Available keys in first sample: {list(features[0].keys())}"
                )
            else:
                ...
                print(
                    f"[Warning] Field '{name}' missing in some batch samples, stacking available ones."
                )
...
class AdvTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if hasattr(outputs, "logits"):
            ...
            try:
                ...
                raise AttributeError(
                    "Could not find 'logits' attribute or access logits as the first element in the model output."
                )
        ...
        if len(logits.shape) != 3:
            raise ValueError(
                f"Expected model output logits to have 3 dimensions [batch, seq_len, vocab_size], but got shape {logits.shape}"
            )
        ...
        idx_o = torch.arange(
            0, last_token_logits.size(0), 2, device=last_token_logits.device
        )
        idx_a = idx_o + 1
        ...
        if (
            len(idx_a) > last_token_logits.size(0) // 2
            or last_token_logits.size(0) % 2 != 0
        ):
            print(
                f"Warning: Odd batch size ({last_token_logits.size(0)}) or mismatched pairs detected in KL loss calculation. Skipping KL for this batch."
            )
            ...
            p = F.log_softmax(
                last_token_logits[idx_o], dim=-1
            )  # Original samples log-probabilities
            q = F.softmax(
                last_token_logits[idx_a], dim=-1
            )  # Adversarial samples probabilities
            loss_kl = F.kl_div(
                p, q, reduction="batchmean", log_target=False
            )  # Use log_target=False as q is softmax

        # Ensure loss_kl is a scalar
        # 确保 loss_kl 是一个标量
        if not isinstance(loss_kl, torch.Tensor) or loss_kl.numel() != 1:
            raise TypeError(
                f"Calculated KL loss is not a scalar tensor. Got: {loss_kl}"
            )

        dummy = (
            last_token_logits.sum() * 0.0
        )  # Dummy tensor for graph connection if needed
        # 如果需要，用于图形连接的虚拟张量
        ...
        return (
            (total_loss, {"logits": last_token_logits})
            if return_outputs
            else total_loss
        )
...
def get_args():
    ...
    ap.add_argument(
        "--phaseA_dir",
        default="phaseA_llama3B_L3prompt",
        help="Phase A LoRA 权重目录（留空=不用）",
    )
    ap.add_argument(
        "--pairs_csv",
        default="./scripts_csv/train.csv",
        help="包含样本对的 CSV 文件路径 (应包含 agreed_labels 列)",
    )  # 更新帮助信息
    ap.add_argument(
        "--output_dir",
        default="./phaseB_new_llama3B_L3prompt_fromA_KLonly",  # 更新默认输出目录名
        help="保存 Phase B 权重的目录",
    )
    # --- 结束修改 ---
    ap.add_argument("--alpha", type=float, default=1.0, help="KL loss 系数")
    ap.add_argument(
        "--batch", type=int, default=8, help="批次大小 (应为偶数以进行样本对处理)"
    )  # 添加提示
    ...
    if args.batch % 2 != 0:
        print(
            f"Warning: Batch size ({args.batch}) is not even. KL divergence loss requires paired samples. Consider using an even batch size."
        )
...
if __name__ == "__main__":
    ...
    if len(train_ds) == 0:
        raise ValueError(
            f"Dataset loaded from '{cfg.pairs_csv}' is empty after filtering for agreed_labels == 0.0. Cannot proceed with training."
        )
    ...
    # 严格检查存在性后再进行转换
    if "is_adv" in train_ds.column_names and pd.api.types.is_numeric_dtype(
        train_ds["is_adv"]
    ):
        columns_to_cast.append("is_adv")
    elif "is_adv" in train_ds.column_names:
        print(
            f"Warning: Column 'is_adv' found but is not numeric. Skipping casting to int64."
        )

    if "pair_id" in train_ds.column_names and pd.api.types.is_numeric_dtype(
        train_ds["pair_id"]
    ):
        columns_to_cast.append("pair_id")
    elif "pair_id" in train_ds.column_names:
        print(
            f"Warning: Column 'pair_id' found but is not numeric. Skipping casting to int64."
        )
    ...
    for col in columns_to_cast:
        try:
            ...
            print(
                f"Warning: Could not cast column '{col}' to int64. Error: {e}. Check column contents in '{cfg.pairs_csv}'."
            )
    ...
    def tok_fn(examples):
        ...
        if "text" not in examples:
            raise KeyError(
                "The input data is missing the required 'text' column for tokenization."
            )
        ...
        tokens["labels"] = [-100] * len(
            tokens["input_ids"]
        )  # Set all labels to -100 as CE loss is zeroed out
        ...
        if "pair_id" in examples:
            keys_to_keep.append("pair_id")
        if "is_adv" in examples:
            keys_to_keep.append("is_adv")
        # --- 结束修改 ---
        ...
        for key in keys_to_keep:
            if key in examples:
                ...
                if isinstance(examples[key], list):
                    ...
                    tokens[key] = [examples[key]] * len(
                        examples["text"]
                    )  # Repeat value for each text in batch
                    # 对批次中的每个文本重复值
    ...
    # 仔细确定要移除的列
    columns_to_remove = [
        col for col in train_ds.column_names if col not in ["text", "pair_id", "is_adv"]
    ]  # Keep text for tok_fn
    # 保留 text 以供 tok_fn 使用
    print(
        f"Columns to remove before map: {columns_to_remove}"
    )  # Debug: 打印映射前要移除的列
    ...
    columns_to_remove_after_map = [
        col for col in train_ds.column_names if col not in final_columns
    ]
    print(
        f"Columns to remove after map: {columns_to_remove_after_map}"
    )  # Debug: 打印映射后要移除的列
    if columns_to_remove_after_map:
        train_ds = train_ds.remove_columns(columns_to_remove_after_map)

    train_ds.set_format(type="torch", columns=final_columns)
    ...
    train_ds = train_ds.filter(
        lambda ex: all(k in ex and ex[k] is not None for k in required_fields)
    )
    if len(train_ds) < original_len_before_filter:
        print(
            f"Warning: Filtered out {original_len_before_filter - len(train_ds)} samples missing required fields after tokenization."
        )
    ...
    if len(train_ds) == 0:
        raise ValueError(
            "Dataset became empty after tokenization or final filtering. Cannot proceed."
        )
    ...
    if cfg.phaseA_dir and os.path.exists(cfg.phaseA_dir):
        ...
        print(
            "No Phase A LoRA directory specified or found. Using base model directly."
        )
    ...
    if trainable_params == 0:
        print(
            "WARNING: No LoRA parameters found or set to trainable. Check adapter loading and PEFT setup."
        )
