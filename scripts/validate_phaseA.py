...
def batch_predict_generate(prompts, bs=64):
    ...
    max_len_supported = len(tok.tokenize("SUPPORTED")) + 2 # Buffer for EOS or other variations
    ...
    max_new_tokens = max(max_len_supported, max_len_refuted, 5) # Min 5 tokens for safety margin
    ...
    for i in range(0, len(prompts), bs):
        ...
        for k in range(generated_ids.shape[0]):
            ...
            decoded_text = tok.decode(generated_part_ids, skip_special_tokens=True).strip()
...
print(f"Clean dev Accuracy: {acc * 100:.2f}%   ({correct_predictions}/{total_processed})")
if other_predictions > 0:
    print(f"Number of 'OTHER' predictions (neither SUPPORTED nor REFUTED detected): {other_predictions} out of {total_processed}")
