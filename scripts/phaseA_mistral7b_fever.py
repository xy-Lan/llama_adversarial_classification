...
def prepare_dataset(
    ...
    current_sys_msg = (
        mistral_system_message_train_nei if keep_nei else mistral_system_message_train
    )
    ...
    def convert(ex):
        ...
        if lab == "NOT ENOUGH INFO":
            ...
            ev, y_text = (
                wiki.sent(ex["evidence_id"], ex["evidence_sentence_id"]),
                "SUPPORTED",
            )
        elif lab == "REFUTES":
            ev, y_text = (
...
def build_trainer(cfg) -> SFTTrainer:
    ...
    tok = AutoTokenizer.from_pretrained(
        cfg.model_id, add_eos_token=False, use_fast=False  # 文本已手动附加 EOS
    )
    ...
    print(
        f"PAD token: '{tok.pad_token}' (ID={tok.pad_token_id}), side={tok.padding_side}"
    )
    print("--------------------------\n")
...
def cli():
    ...
    ap.add_argument(
        "--output_dir",
        default="./phaseA_mistral7B_fever",
